  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="address in addressList" :key="address.id"
           @click="changeDefaultAddress(address, addressList)">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="commodity in tradeInfo.detailArrayList" :key="commodity.skuId">
          <li>
            <img :src="commodity.imgUrl" alt="cover" style="width: 100px; height: auto">
          </li>
          <li>
            <p>{{ commodity.skuName }}</p>
            <h4>7天无理由退货</h4>
          </li>
          <li>
            <h3>￥ {{ commodity.orderPrice }}</h3>
          </li>
          <li>X {{ commodity.skuNum }}</li>
          <li>有货</li>
        </ul>
      </div>
      <div class="bbs">
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{ selectedAddress.fullAddress }}湖北省武汉市</span>
        收货人：<span>{{ selectedAddress.consignee }}Bear</span>
        电话：<span>{{ selectedAddress.phoneNum }}13596485684</span>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async submitOrder() {
      ...
      let data = {
        "consignee": "Spring-_-Bear",
        "consigneeTel": "13596485684",
        "deliveryAddress": "湖北省武汉市",
        "paymentWay": "ONLINE",
      ...
      if (result.code === 200) {
        ...
        this.$router.push('/order/pay?orderId=' + orderId)
      } else {
